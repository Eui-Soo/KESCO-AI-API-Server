...
`TARGET_DATE_OFFSET_DAYS`는 스케줄러가 처리할 데이터 날짜를 정합니다.
...
예를 들어 매일 `00:00:30`에 실행하는 경우, 당일 데이터는 아직 충분히 쌓이지 않았을 수 있으므로 운영에서는 `-1`로 설정하여 전날 데이터를 처리하는 방식을 권장합니다.
...
현재 `/api/v1/pipeline/run` 수동 실행 API도 `TARGET_DATE_OFFSET_DAYS` 값을 그대로 반영합니다. 따라서 `TARGET_DATE_OFFSET_DAYS=-1`이면 수동 실행 시에도 전날 데이터를 기준으로 처리합니다.
...
본 프로젝트는 ㈜로보볼트의 ESS 배터리 데이터 분석 및 AI 이상탐지 기술을 기반으로, 전기안전 분야 관제시스템과의 연동을 목표로 개발 중인 백엔드 AI API 서버입니다.

관제시스템에서 수집되는 ESS 운용 데이터를 AI 서버가 주기적으로 분석하고, Rack 및 Cell 단위의 이상 점수와 위험 등급을 산출하여 관제시스템에서 활용할 수 있도록 API 형태로 제공합니다.

본 저장소는 로보볼트와 전기안전 유관기관 간 관제시스템 연동 및 AI 분석 기능 검증을 위한 개발 저장소이며, 실제 운영 환경에서는 관제시스템 DB 구조, 현장 데이터 포맷, AI 모델 버전, 보안 정책에 맞춰 추가 수정 및 검증이 필요합니다.